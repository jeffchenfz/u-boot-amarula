 ...
 */
u64 sdram_get_cs_cap(struct rk3399_cap_info *cap_info, u32 cs, u32 dram_type)
{
	u32 bg;
	u64 cap[2];
...
void sdram_print_ddr_info(struct rk3399_cap_info *cap_info,
			  struct rk3399_base_params *base, u32 split)
{
	u64 cap;
	u32 bg;
...
void sdram_org_config(struct rk3399_cap_info *cap_info,
...
void sdram_msch_config(struct rk3399_msch_regs *msch,
...
int sdram_detect_col(struct rk3399_cap_info *cap_info,
		     u32 coltmp)
{
...
int sdram_detect_bank(struct rk3399_cap_info *cap_info,
...
/* detect bg for ddr4 */
int sdram_detect_bg(struct rk3399_cap_info *cap_info,
		    u32 coltmp)
{
	void __iomem *test_addr;
	u32 dbw;
...
int sdram_detect_dbw(struct rk3399_cap_info *cap_info, u32 dram_type)
{
...
int sdram_detect_row(struct rk3399_cap_info *cap_info,
...
int sdram_detect_row_3_4(struct rk3399_cap_info *cap_info,
			 u32 coltmp, u32 bktmp)
{
	u32 row_3_4;
	u32 bw = cap_info->bw;
...
int sdram_detect_cs1_row(struct rk3399_cap_info *cap_info, u32 dram_type)
{
	void __iomem *test_addr;
		    ...
		    struct rk3399_sdram_params *sdram_params)
{
	...
	copy_to_reg(&denali_phy[128],
			  &params_phy[128], (218 - 128 + 1) * 4);
	copy_to_reg(&denali_phy[256],
			  &params_phy[256], (346 - 256 + 1) * 4);
	copy_to_reg(&denali_phy[384],
			  &params_phy[384], (474 - 384 + 1) * 4);
	copy_to_reg(&denali_phy[512],
			  &params_phy[512], (549 - 512 + 1) * 4);
	copy_to_reg(&denali_phy[640],
			  &params_phy[640], (677 - 640 + 1) * 4);
	copy_to_reg(&denali_phy[768],
			   ...
			   unsigned char channel)
{
	...
	struct rk3399_cap_info *cap_info = &sdram_params->ch[channel].cap_info;
	u32 bw;
	u32 col_tmp;
	u32 bk_tmp;
	u32 row_tmp;
			     ...
			     u32 channel)
{
	 ...
	 */
	copy_to_reg((u32 *)&denali_phy[59],
			  (u32 *)&denali_phy_params[59],
			  (63 - 58) * 4);
	copy_to_reg((u32 *)&denali_phy[187],
			  (u32 *)&denali_phy_params[187],
			  (191 - 186) * 4);
	copy_to_reg((u32 *)&denali_phy[315],
			  (u32 *)&denali_phy_params[315],
			  (319 - 314) * 4);
	copy_to_reg((u32 *)&denali_phy[443],
		      ...
		      struct rk3399_sdram_params *sdram_params)
{
	...
	ctl_fn = 0;
	phy_fn = get_phy_fn(sdram_params, ctl_fn);
